...
def fk(theta1, theta2, theta3):   #defining a function which is for calculating forward kinematics
...
# draw initial arm
base, j1, j2, ee = fk(theta1_0, theta2_0, theta3_0) # calling the fk function to get the positions of base, joint and end-effector

(link_line,) = ax.plot([base[0], j1[0], j2[0], ee[0]], # plotting the arm
                       [base[1], j1[1], j2[1], ee[1]],
...
def update(_):
    ...
    b, j1, j2, e = fk(th1, th2, th3) # calling the fk function to get the positions of base, joint and end-effector

    link_line.set_data([b[0], j1[0], j2[0], e[0]], [b[1], j1[1], j2[1], e[1]]) # updating the arm position
    ee_text.set_text(f"EE: x={e[0]:.3f}, y={e[1]:.3f} \nθ1={np.rad2deg(th1):.1f}°, θ2={np.rad2deg(th2):.1f}° θ3={np.rad2deg(th3):.1f}°") # updating the end-effector text    
    plt.draw()
