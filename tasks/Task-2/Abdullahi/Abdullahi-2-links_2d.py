...
def fk(theta1, theta2):
    ...
    x2 = x1 + L2*np.cos(theta1 + theta2)  # Calculate x-coordinate of the end-effector (end of link 2)
    y2 = y1 + L2*np.sin(theta1 + theta2)  # Calculate y-coordinate of the end-effector (end of link 2)
...
def update(_):
    ...
    b, j, e = fk(th1, th2)          # Compute positions: base, joint, and end-effector
    link_line.set_data([b[0], j[0], e[0]], [b[1], j[1], e[1]])   # Update the robot arm plot
